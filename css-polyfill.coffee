...
class ClassRenamer extends AbstractSelectorVisitor
  ...
  visitSelectorOut: (node, visitArgs) ->
    ...
    frame.selectors ?= {}
    frame.selectors[newClassName] = newElements
  ...
  operateOnElements: (frame, $els, node) ->
    ...
    if frame.pseudoSelectors
      ...
      _.each node.selectors, (selector) ->
        debugSelectors.push(selector.toCSS({}))
...
class CSSCanonicalizer
  ...
  visit: ($node) ->
    ...
    if newClass
      ...
      @newAutogenClasses[newClassName] = new AutogenClass("CANONICALIZED from #{prevClassesStr}", newRules)
