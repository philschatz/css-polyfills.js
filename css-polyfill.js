...
(function() {
  var AbstractSelectorVisitor, ClassRenamer, ClassRenamerPlugin, CounterDomVisitor, CounterLessVisitor, CounterPlugin, DomVisitor, LessVisitor, MoveToDomVisitor, MoveToLessVisitor, MoveToPlugin, PSEUDO_ELEMENT_NAME, Plugin, PseudoExpanderPlugin, PseudoSelectorExpander, RemoveDisplayNone, RemoveDisplayNonePlugin, SelectorValueNode, SetContentDomVisitor, SetContentPlugin, SetContentVisitor, TargetCounterPlugin, TargetTextPlugin, ValueNode, findBefore, freshClass, freshClassIdCounter, numberingStyle, parseCounters, toRoman,
    ...
    AbstractSelectorVisitor.prototype.dataAppendAll = function($els, dataKey, exprs) {
      ...
      if (exprs) {
        if (!(exprs instanceof Array)) {
          console.error('BUG: dataAppendAll takes an array of expressions');
        }
  ...
  TargetCounterPlugin = {
    definedFunctions: {
      ...
      'target-counter': function(id, counterName, counterStyle) {
        ...
        id = id.value;
        if (id[0] !== '#') {
          console.error('BUG: target-id MUST start with #');
        }
        $target = $(id);
        if ($target.length) {
          val = 0;
      ...
      'target-text': function(id, whichNode) {
        ...
        if (whichNode) {
          if ('content' !== whichNode.name) {
            console.error('BUG: must be a content(...) call');
          }
        ...
        switch (whichText) {
          ...
          case 'first-letter':
            ...
            break;
          default:
            console.error('BUG! invalid whichText');
        }
